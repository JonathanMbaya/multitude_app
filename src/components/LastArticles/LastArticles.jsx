...
function LastArticles() {
    ...
    useEffect(() => {
        const fetchLatestArticles = async () => {
            try {
                const response = await axios.get(`${import.meta.env.VITE_API_BASE_URL}/articles/last/latest`);
    ...
    if (loading) return <div><hr style={{ maxWidth: '300px' }} /><LoadingPage /></div>;
            ...
            {articles.map((article) => (
                <div className="post-article-home" key={article.id}>
                    ...
                    <div className="post-text-home">
                        <span className="text-category">{getCategoryName(article.category)}</span>
                        <h2>{article.titre}</h2>
                        <p>{article.extrait}</p>
